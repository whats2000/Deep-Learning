...
import src_yolo_v5.config as config

# ALADDIN'S MODIFIED
def intersection_over_union(boxes_preds, boxes_labels, box_format="midpoint", g_iou=False, eps=1e-7):
    """
    ...
    Parameters:
        ...
        g_iou (bool): if True it computed GIoU loss (https://giou.stanford.edu)
...
class ComputeLoss:
    ...
    def __init__(self, model, save_logs=False, filename=None, resume=False):
        ...
        self.lambda_obj = 1 * ((config.YOLO_IMG_DIM / 640) ** 2 * 3 / model.head.nl)
    ...
    def __call__(self, p, targets, pred_size, batch_idx=None, epoch=None):  # predictions, targets
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                iou = intersection_over_union(pbox, tbox[i], g_iou=True).squeeze()  # iou(prediction, target)
